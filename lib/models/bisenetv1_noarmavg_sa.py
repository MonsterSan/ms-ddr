...
class ContextPath(nn.Module):
    def __init__(self, *args, **kwargs):
        ...
        self.conv_feat64 = ConvBNReLU(512,1024,ks=3,stride=2, padding=1)
        self.conv_head64 = ConvBNReLU(1024,128,1,1,0)
        self.conv_head32 = ConvBNReLU(128, 128, ks=3, stride=1, padding=1)
        self.conv_head16 = ConvBNReLU(128, 128, ks=3, stride=1, padding=1)
        self.conv_avg = ConvBNReLU(512, 128, ks=1, stride=1, padding=0)
        self.up32 = nn.Upsample(scale_factor=2.)
        self.up16 = nn.Upsample(scale_factor=2.)
        self.up64 = nn.Upsample(scale_factor=2.)
        self.shortaspp = ASPP(128,128)

        self.init_weight()

    def forward(self, x):
        feat8, feat16, feat32 = self.resnet(x)
        feat64 = self.conv_feat64(feat32)
        feat64 = self.conv_head64(feat64)
        feat64 = self.up64(feat64)
        feat64 = self.shortaspp(feat64)

        feat32_arm = self.arm32(feat32)
        feat32_sum = feat32_arm + feat64
...
class BiSeNetV1_noarmavg_sa(nn.Module):

    def __init__(self, n_classes, aux_mode='train', *args, **kwargs):
        super(BiSeNetV1_noarmavg_sa, self).__init__()
        self.cp = ContextPath()
        self.sp = SpatialPath()
