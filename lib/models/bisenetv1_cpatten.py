...
class cpattention(nn.Module):
    def __init__(self, in_ch, out_ch):
        super(cpattention, self).__init__()
        self.conv = ConvBNReLU(in_ch, out_ch, ks=3, stride=1, padding=1)
        self.conv_atten = nn.Conv2d(out_ch, out_ch, kernel_size=1, bias=False)
        self.bn_atten = BatchNorm2d(out_ch)
...
class BiSeNetV1_cpatten(nn.Module):

    def __init__(self, n_classes, aux_mode='train', *args, **kwargs):
        super(BiSeNetV1_cpatten, self).__init__()
        self.cp = ContextPath()
        self.sp = SpatialPath()
