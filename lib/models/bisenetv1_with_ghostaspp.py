...
class ASPP(nn.Module):
    def __init__(self, in_channels, out_channels, atrous_rates=None):
        ...
        self.branch1ghost = nn.Sequential(
        ...
        self.branch2 = nn.Sequential(
            nn.Conv2d(in_channels, out_channels // 2, 3, padding=atrous_rates[0], dilation=atrous_rates[0], bias=False),
            nn.BatchNorm2d(out_channels // 2),
            nn.ReLU()
        )
        self.branch2ghost = nn.Sequential(
            nn.Conv2d(out_channels // 2, out_channels // 2, kernel_size=3, stride=1, padding=1,
                      groups=out_channels // 2, bias=False),
            nn.BatchNorm2d(out_channels // 2),
            nn.ReLU(inplace=True)
        ...
        self.branch3 = nn.Sequential(
            nn.Conv2d(in_channels, out_channels // 2, 3, padding=atrous_rates[1], dilation=atrous_rates[1], bias=False),
            nn.BatchNorm2d(out_channels // 2),
            nn.ReLU()
        )
        self.branch3ghost = nn.Sequential(
            nn.Conv2d(out_channels // 2, out_channels // 2, kernel_size=3, stride=1, padding=1,
                      groups=out_channels // 2, bias=False),
            nn.BatchNorm2d(out_channels // 2),
            nn.ReLU(inplace=True)
        ...
        self.branch4 = nn.Sequential(
            nn.Conv2d(in_channels, out_channels // 2, 3, padding=atrous_rates[2], dilation=atrous_rates[2], bias=False),
            nn.BatchNorm2d(out_channels // 2),
            nn.ReLU()
        )
        self.branch4ghost = nn.Sequential(
            nn.Conv2d(out_channels // 2, out_channels // 2, kernel_size=3, stride=1, padding=1,
                      groups=out_channels // 2, bias=False),
            nn.BatchNorm2d(out_channels // 2),
            nn.ReLU(inplace=True)
        ...
        self.branch5ghost = nn.Sequential(
            nn.Conv2d(out_channels // 2, out_channels // 2, kernel_size=1, groups=out_channels // 2, bias=False),
            nn.BatchNorm2d(out_channels // 2),
            nn.ReLU(inplace=True)
        )
    ...
    def forward(self, x):
        # res = []
        out1 = self.branch1(x)
        ghost1 = self.branch1ghost(out1)
        out2 = self.branch2(x)
        ghost2 = self.branch2ghost(out2)
        out3 = self.branch3(x)
        ghost3 = self.branch3ghost(out3)
        out4 = self.branch4(x)
        ghost4 = self.branch4ghost(out4)
        out5 = self.branch5(x)
        ghost5 = self.branch5ghost(out5)
        size = x.shape[-2:]
        out5 = F.interpolate(out5, size=size, mode='bilinear', align_corners=False)
        ghost5 = F.interpolate(ghost5, size=size, mode='bilinear', align_corners=False)
        out = torch.cat([out1, ghost1, out2, ghost2, out3, ghost3, out4, ghost4, out5, ghost5], dim=1)
...
class BiSeNetV1_ghostaspp(nn.Module):

    def __init__(self, n_classes, aux_mode='train', *args, **kwargs):
        super(BiSeNetV1_ghostaspp, self).__init__()
        self.cp = ContextPath()
        self.sp = SpatialPath()
