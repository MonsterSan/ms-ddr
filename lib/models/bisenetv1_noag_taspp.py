...
class ASPP(nn.Module):
    def __init__(self, in_channels, out_channels, atrous_rates=None):
        ...
        self.split = in_channels // 4
        # 1*1 branch
        self.branch1 = nn.Sequential(
            nn.Conv2d(in_channels // 4, out_channels // 4, 1, bias=False),
        ...
        self.branch2 = nn.Sequential(
            nn.Conv2d(in_channels // 4, out_channels // 4, 3, padding=atrous_rates[0], dilation=atrous_rates[0],
                      bias=False),
            nn.BatchNorm2d(out_channels // 4),
            nn.ReLU()
        )
        ...
        self.branch3 = nn.Sequential(
            nn.Conv2d(in_channels // 4, out_channels // 4, 3, padding=atrous_rates[1], dilation=atrous_rates[1],
                      bias=False),
            nn.BatchNorm2d(out_channels // 4),
            nn.ReLU()
        )
        ...
        self.branch4 = nn.Sequential(
            nn.Conv2d(in_channels // 4, out_channels // 4, 3, padding=atrous_rates[2], dilation=atrous_rates[2],
                      bias=False),
            nn.BatchNorm2d(out_channels // 4),
            nn.ReLU()
        )
...
class ContextPath(nn.Module):
    def __init__(self, *args, **kwargs):
        ...
        # self.aspp16 = AttentionRefinementModule(256, 128)
        self.aspp16 = ASPP(256, 128)
        # self.aspp32 = AttentionRefinementModule(512, 128)
        self.aspp32 = ASPP(512, 128)
    ...
    def forward(self, x):
        ...
        feat32_sum = self.aspp32(feat32)
        feat32_up = self.up32(feat32_sum)
        feat32_up = self.conv_head32(feat32_up)

        feat16_aspp = self.aspp16(feat16)
        feat16_sum = feat16_aspp + feat32_up
...
class BiSeNetV1_noag_taspp(nn.Module):

    def __init__(self, n_classes, aux_mode='train', *args, **kwargs):
        super(BiSeNetV1_noag_taspp, self).__init__()
        self.cp = ContextPath()
        self.sp = SpatialPath()
