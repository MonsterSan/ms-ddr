...
class CaSv2(nn.Module):
    def __init__(self):
        ...
        self.cpavg = nn.AdaptiveAvgPool2d(1)
        self.cpmax = nn.AdaptiveMaxPool2d(1)
        self.spavg = nn.AdaptiveAvgPool2d(1)
        self.spmax = nn.AdaptiveMaxPool2d(1)
        self.sigmoid = nn.Sigmoid()

    def forward(self, sp8, cp16):
        cpmax = self.cpmax(cp16)
        cpavg = self.cpavg(cp16)
        spmax = self.spmax(sp8)
        spavg = self.spavg(sp8)
...
class BiSeNetV1_noarm_global2taspp_ffm2mix_v2(nn.Module):

    def __init__(self, n_classes, aux_mode='train', *args, **kwargs):
        super(BiSeNetV1_noarm_global2taspp_ffm2mix_v2, self).__init__()
        self.cp = ContextPath()
        self.sp = SpatialPath()
        self.CaS = CaSv2()
...
if __name__ == "__main__":
    net = BiSeNetV1_noarm_global2taspp_ffm2mix_v2(2)
    net.cuda()
    net.eval()
