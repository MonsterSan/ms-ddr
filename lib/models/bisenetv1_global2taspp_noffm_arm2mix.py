...
class MixAtten(nn.Module):
    ...
    def forward(self, x, y):
        global_x = torch.mean(x, dim=(2, 3), keepdim=True)
        global_y = torch.mean(y, dim=(2, 3), keepdim=True)
        atten = self.conv1(torch.cat([global_x, global_y], dim=1))
        atten = self.bn(atten)
        atten = atten.sigmoid()
        return x * atten, y * (1-atten)
...
class ContextPath(nn.Module):
    def __init__(self, *args, **kwargs):
        ...
        self.conv16 = ConvBNReLU(256, 128, ks=1, stride=1, padding=0, bias=False)
    ...
    def forward(self, x):
        ...
        feat32_up, feat16_conv = self.mix(feat32_up, feat16_conv)
...
class BiSeNetV1_global2taspp_noffm_arm2mix(nn.Module):

    def __init__(self, n_classes, aux_mode='train', *args, **kwargs):
        super(BiSeNetV1_global2taspp_noffm_arm2mix, self).__init__()
        self.cp = ContextPath()
        self.sp = SpatialPath()
...
if __name__ == "__main__":
    net = BiSeNetV1_global2taspp_noffm_arm2mix(2)
    net.cuda()
    net.eval()
