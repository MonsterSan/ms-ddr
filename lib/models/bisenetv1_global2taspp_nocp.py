...
class BiSeNetV1_global2taspp_nocp(nn.Module):

    def __init__(self, n_classes, aux_mode='train', *args, **kwargs):
        super(BiSeNetV1_global2taspp_nocp, self).__init__()
        self.cp = ContextPath()
        self.sp = SpatialPath()
    ...
    def forward(self, x):
        H, W = x.size()[2:]
        feat_sp = self.sp(x)
        feat_cp8, feat_cp16 = self.cp(feat_sp)

        # print("sp {}".format(feat_sp.shape))
        # 16 128 64 64
...
if __name__ == "__main__":
    net = BiSeNetV1_global2taspp_nocp(2)
    net.cuda()
    net.eval()
