...
class CaS(nn.Module):
    ...
    def forward(self, sp8, cp16):
        cp16 = self.up(cp16)
        cpmax, _ = torch.max(cp16, dim=1, keepdim=True)
...
class BiSeNetV1_noarm_global2taspp_ffm2mix(nn.Module):

    def __init__(self, n_classes, aux_mode='train', *args, **kwargs):
        super(BiSeNetV1_noarm_global2taspp_ffm2mix, self).__init__()
        self.cp = ContextPath()
        self.sp = SpatialPath()
        self.CaS = CaS()
...
if __name__ == "__main__":
    net = BiSeNetV1_noarm_global2taspp_ffm2mix(2)
    net.cuda()
    net.eval()
