...
class FeatureFusionModule(nn.Module):
    def __init__(self, in_chan, out_chan, *args, **kwargs):
        ...
        self.init_weight()
        self.conv_atten = nn.Conv2d(in_chan//2, in_chan//2, kernel_size= 1, bias=False)
        self.bn_atten = BatchNorm2d(in_chan//2)

    def forward(self, fsp, fcp):
        atten = torch.mean(fsp+fcp, dim=(2, 3), keepdim=True)
        atten = self.conv_atten(atten)
        atten = self.bn_atten(atten)
        fcp = torch.mul(fcp, atten)
...
class BiSeNetV1_global2taspp_ffmarm(nn.Module):

    def __init__(self, n_classes, aux_mode='train', *args, **kwargs):
        super(BiSeNetV1_global2taspp_ffmarm, self).__init__()
        self.cp = ContextPath()
        self.sp = SpatialPath()
...
if __name__ == "__main__":
    net = BiSeNetV1_global2taspp_ffmarm(2)
    net.cuda()
    net.eval()
